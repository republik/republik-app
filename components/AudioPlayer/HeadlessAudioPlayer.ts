...
const HeadlessAudioPlayer = ({}) => {
  ...
  const [playbackRate, setPlaybackRate] = useState(persistedState.playbackRate || 1);
  ...
  useInterval(
    () => syncStateWithWebUI(),
    playerState.state && [State.Buffering, State.Playing].includes(playerState.state)
  ...
  useEffect(() => {
    if (persistedState.playbackRate && persistedState.playbackRate !== playbackRate) {
      console.log("Restoring playback rate from persisted state:", persistedState.playbackRate);
      ...
      if (isInitialized) {
        TrackPlayer.setRate(persistedState.playbackRate).catch((error) => {
          console.error("Failed to restore playback rate to TrackPlayer:", error);
        });
      }
    }
